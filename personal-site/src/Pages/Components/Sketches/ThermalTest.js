...
const ThermalTest = (p) => {
  ...
  p.setup = () => {
    ...
    console.log("ThermalTest setup complete. Click to connect magic.");
  ...
  p.draw = () => {
    ...
    for (let y = 0; y < gridSize; y++) {
      for (let x = 0; x < gridSize; x++) {
        ...
        // Get temperature from thermal sensor if available
        if (isMagic && magic.modules.thermal?.raw?.pixel_temperatures) {
          const thermalData = magic.modules.thermal.raw.pixel_temperatures;
          // Map our 10x10 grid to the 8x8 thermal grid
          const thermalX = p.floor(p.map(x, 0, gridSize, 0, 8, true));
          const thermalY = p.floor(p.map(y, 0, gridSize, 0, 8, true));
          const tempIndex = thermalY * 8 + thermalX;
          temperature = thermalData[tempIndex] || 25;
        }
        
        // Map temperature to color (20°C to 40°C)
        const normalizedTemp = p.map(temperature, 20, 40, 0, 1, true);
        const r = p.lerp(0, 255, normalizedTemp);
        const g = p.lerp(255, 0, normalizedTemp);
        const b = 0;
        ...
        p.text(`${Math.round(temperature*10)/10}°C`, 
...
// IMPORTANT: Rename 'ThermalTest' to match the filename PascalCase
export default ThermalTest; 